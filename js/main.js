            ...
            (self.store = createStore(reducer)),
                                ...
                                self.accounts[self.selectedAccount].kind
                                    ...
                                    : 'RPC' === self.connection.connection &&
                                      ...
                                      ? self.accounts[self.selectedAccount].pk
                                        ? (self.dialogError(
                                              "You are using a GoatDEX account that has a private key attached, but you're connected to MetaMask. You should disable MetaMask from Chrome's Window -> Extensions menu (don't worry, this won't lose your MetaMask data), then refresh GoatDEX."
                                          ),
                                          ga('send', {
                                              hitType: 'event',
                                              eventCategory: 'Error',
                                      ...
                                      : 'PublicRPC' !==
                                        ...
                                        ? 'PublicRPC' !==
                                          ...
                                          ? 'PublicRPC' ===
                                            ...
                                            ? (self.dialogError(
                                                  "Your wallet's ETH balance (" +
                                                      balance +
                                                      ' ETH) is not enough to cover the gas cost (Ethereum network fee). GoatDEX sends ' +
                                                      self.minGas +
                                                      ...
                                                      self.minGas +
                                                      ' so you can pay for not only this transaction, but also future transactions you do on GoatDEX. The gas has to come directly from your Wallet (GoatDEX has no physical way of paying gas from your deposited ETH).'
                                              ),
                                              ga('send', {
                                                  hitType: 'event',
                                                  eventCategory: 'Error',
                                          ...
                                          : (self.dialogError(
                                                'You are using a GoatDEX account that has an invalid private key.'
                                            ),
                                            ga('send', {
                                                hitType: 'event',
                                                eventCategory: 'Error',
                                        ...
                                        : (self.dialogError(
                                              'You are using a GoatDEX account that doesn\'t have a private key attached. Perhaps you created the account using MetaMask, in which case you should make sure MetaMask is enabled and logged in to this account, then refresh GoatDEX. Or, if you have the private key, you can choose "Import account" from the accounts dropdown (upper right) to re-import the account with its private key.'
                                          ),
                                          ga('send', {
                                              hitType: 'event',
                                              eventCategory: 'Error',
                ...
                (self.deleteAccount = function() {
                    if (self.accounts[self.selectedAccount].pk) {
                        ...
                        alertify.dialogInfo(
                            'This account is not stored on GoatDEX. Nothing to forget!'
                        );
                }),
                ...
                (self.updateUrl = function() {
                    ...
                    0 ===
                        ...
                        (baseName = baseName ? baseName : baseAddr),
                        (window.location.hash =
                            '#!/trade/' + tokenName + '-' + baseName),
                        (window.document.title =
                            'GoatDEX ' + tokenName + '/' + baseName);
                }),
                ...
                (self.checkContractUpgrade = function() {
                    self.selectedContract &&
                        ...
                        self.dialogInfo(
                            '<p>GoatDEX has a new smart contract. It is now selected.</p><p>Please use the "Smart Contract" menu to select the old one and withdraw from it.</p><p><a href="javascript:;" class="btn btn-default" onclick="alertify.closeAll();</p>'
                        );
                }),
                ...
                (self.refresh = function(callback, initMarket, token, base) {
                    self.q.push(function(done) {
                        ...
                        'ETH' !== self.selectedBase.name &&
                            ((self.selectedBase = self.config.bases[0]),
                            self.store.dispatch({
                                type: 'UPDATE_SELECTED_BASE',
                                value: self.selectedBase,
                            }),
